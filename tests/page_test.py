...
class TestPageSetUp(unittest.TestCase):
    ...
    def test_redirect_true(self):
        ...
        self.assertEqual(
            mp.url, "http://en.wikipedia.org/wiki/Edison,_New_Jersey")
    ...
    def test_disambiguate(self):
        ...
        try:
            ram = wikipedia.page(
                "Dodge Ram (disambiguation)", auto_suggest=False, redirect=False)
            error_raised = False
        ...
        self.assertEqual(options, [u'Dodge Ramcharger', u'Dodge Ram Van', u'Dodge Mini Ram', u'Dodge Caravan C/V',
                                   u'Dodge Caravan C/V', u'Ram C/V', u'Dodge Ram 50', u'Dodge D-Series', u'Dodge Rampage', u'Ram (brand)'])
    ...
    def test_auto_suggest(self):
        ...
        self.assertEqual(
            butterfly.url, "http://en.wikipedia.org/wiki/Butterfly")
...
class TestPage(unittest.TestCase):
    ...
    def test_url(self):
        """Test the url."""
        self.assertEqual(
            self.celtuce.url, "http://en.wikipedia.org/wiki/Celtuce")
        self.assertEqual(
            self.cyclone.url, "http://en.wikipedia.org/wiki/Tropical_Depression_Ten_(2005)")

    def test_content(self):
        """Test the plain text content."""
        self.assertEqual(
            self.celtuce.content, mock_data['data']["celtuce.content"])
        self.assertEqual(
            self.cyclone.content, mock_data['data']["cyclone.content"])

    def test_revision_id(self):
        """Test the revision id."""
        self.assertEqual(
            self.celtuce.revision_id, mock_data['data']["celtuce.revid"])
        self.assertEqual(
            self.cyclone.revision_id, mock_data['data']["cyclone.revid"])

    def test_parent_id(self):
        """Test the parent id."""
        self.assertEqual(
            self.celtuce.parent_id, mock_data['data']["celtuce.parentid"])
        self.assertEqual(
            self.cyclone.parent_id, mock_data['data']["cyclone.parentid"])

    def test_summary(self):
        """Test the summary."""
        self.assertEqual(
            self.celtuce.summary, mock_data['data']["celtuce.summary"])
        self.assertEqual(
            self.cyclone.summary, mock_data['data']["cyclone.summary"])

    def test_images(self):
        """Test the list of image URLs."""
        self.assertEqual(
            sorted(self.celtuce.images), mock_data['data']["celtuce.images"])
        self.assertEqual(
            sorted(self.cyclone.images), mock_data['data']["cyclone.images"])

    def test_references(self):
        """Test the list of reference URLs."""
        self.assertEqual(
            self.celtuce.references, mock_data['data']["celtuce.references"])
        self.assertEqual(
            self.cyclone.references, mock_data['data']["cyclone.references"])

    def test_links(self):
        """Test the list of titles of links to Wikipedia pages."""
        self.assertEqual(
            self.celtuce.links, mock_data['data']["celtuce.links"])
        self.assertEqual(
            self.cyclone.links, mock_data['data']["cyclone.links"])

    def test_html(self):
        """Test the full HTML method."""
        self.assertEqual(
            self.celtuce.html(), mock_data['data']["celtuce.html"])

    def test_sections(self):
        """Test the list of section titles."""
        self.assertEqual(
            sorted(self.cyclone.sections), mock_data['data']["cyclone.sections"])

    def test_section(self):
        """Test text content of a single section."""
        self.assertEqual(
            self.cyclone.section("Impact"), mock_data['data']["cyclone.section.impact"])
    ...
    def test_coordinates(self):
        ...
        self.assertEqual(str(lat.quantize(Decimal('1.000'))), mock_data[
                         'data']['great_wall_of_china.coordinates.lat'])
        self.assertEqual(str(lon.quantize(Decimal('1.000'))), mock_data[
                         'data']['great_wall_of_china.coordinates.lon'])
