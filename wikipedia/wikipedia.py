...
def geosearch(latitude, longitude, title=None, results=10, radius=1000):
    ...
    * radius - Search radius in meters. The value must be between 10 and 10000
    ...
    if search_pages:
        search_results = (v['title']
                          for k, v in search_pages.items() if k != '-1')
    else:
        search_results = (d['title']
                          for d in raw_results['query']['geosearch'])
...
class WikipediaPage(object):
    ...
    def __load(self, redirect=True, preload=False):
        ...
        # then the page must be a disambiguation page
            ...
            raise DisambiguationError(
                getattr(self, 'title', page['title']), may_refer_to)
    ...
    def html(self):
        ...
        if not getattr(self, '_html', False):
            ...
            request = _wiki_request(query_params)
            self._html = request['query']['pages'][
                self.pageid]['revisions'][0]['*']
    ...
    def content(self):
        ...
        if not getattr(self, '_content', False):
            ...
            self._revision_id = request['query']['pages'][
                self.pageid]['revisions'][0]['revid']
            self._parent_id = request['query']['pages'][
                self.pageid]['revisions'][0]['parentid']
    ...
    def coordinates(self):
        ...
        if not getattr(self, '_coordinates', False):
            ...
            if 'query' in request:
                coordinates = request['query'][
                    'pages'][self.pageid]['coordinates']
                self._coordinates = (
                    Decimal(coordinates[0]['lat']), Decimal(coordinates[0]['lon']))
            else:
...
def donate():
    ...
    webbrowser.open(
        'https://donate.wikimedia.org/w/index.php?title=Special:FundraiserLandingPage', new=2)
